...
export default function CreatePaymentCustomerForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const queryParam = {
      date_from: ensureDateTimeFormat(paymentCustomer.dateFrom),
      date_to: ensureDateTimeFormat(paymentCustomer.dateTo),
      customer_id: paymentCustomer.selectedCustomer.id,
    };

    try {
      // Send POST request to the API
      const response = await API.post("/reports/payment-customer", queryParam, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
      ...
      toast.success("success add new paymentCustomer");
      navigate("/paymentCustomer");
    } catch (error) {
  ...
  const fetchPaymentCustomerData = async (fromDate, toDate, customerId) => {
    try {
      const queryParam = {
  ...
  const exportSimpleExcel = async (fromDate, toDate, customerId) => {
    const data = await fetchPaymentCustomerData(fromDate, toDate, customerId);
    if (data.length === 0) {
    ...
    const worksheet = workbook.addWorksheet("Payment Customer Report");

    worksheet.mergeCells("A1:E1");
    const titleCell = worksheet.getCell("A1");
    titleCell.value = "Payment Customer Report";
    ...
    const headerRow = worksheet.addRow(["No", "Payment Customer", "Date", "Customer", "Total"]);
    ...
    saveAs(blob, "Payment Customer.xlsx");
  ...
  const handlePreview = async (fromDate, toDate, customerId) => {
    ...
    navigate("/preview-payment-customer", { state: { reportData: data, fromDate, toDate } });
  };
              ...
              <Grid item xs={4}>
                <MDBox mb={2}>
                  <MDInput
                    ...
                    label="Date From"
                    fullWidth
                    value={paymentCustomer.dateFrom}
                    onChange={(e) =>
                      setPaymentCustomer({ ...paymentCustomer, dateFrom: e.target.value })
                    }
                    InputLabelProps={{
              ...
              <Grid item xs={4}>
                <MDBox mb={2}>
                  <MDInput
                    ...
                    label="Date To"
                    fullWidth
                    value={paymentCustomer.dateTo}
                    onChange={(e) =>
                      setPaymentCustomer({ ...paymentCustomer, dateTo: e.target.value })
                    }
                    InputLabelProps={{
                      shrink: true, // Ensures label stays on top even when the input is empty
                    }}
                ...
                <Grid item xs={2}>
                  <MDBox display="flex" justifyContent="flex-start" mt={3}>
                    <MDButton
                      ...
                      onClick={() =>
                        handlePreview(
                          paymentCustomer.dateFrom,
                          paymentCustomer.dateTo,
