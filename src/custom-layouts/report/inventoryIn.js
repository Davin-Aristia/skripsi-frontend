...
export default function CreateInventoryInForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const queryParam = {
      date_from: ensureDateTimeFormat(inventoryIn.dateFrom),
      date_to: ensureDateTimeFormat(inventoryIn.dateTo),
      vendor_id: inventoryIn.selectedVendor.id,
    };

    try {
      // Send POST request to the API
      const response = await API.post("/reports/inventoryIn", queryParam, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      });
      ...
      toast.success("success add new inventoryIn");
      navigate("/inventoryIn");
    } catch (error) {
  ...
  const fetchInventoryInData = async (fromDate, toDate, vendorId) => {
    try {
      const queryParam = {
      ...
      const response = await API.post("/reports/inventory-in", queryParam, {
        headers: {
  ...
  const exportSimpleExcel = async (fromDate, toDate, vendorId) => {
    const data = await fetchInventoryInData(fromDate, toDate, vendorId);
    if (data.length === 0) {
    ...
    const worksheet = workbook.addWorksheet("Inventory In Report");

    worksheet.mergeCells("A1:E1");
    const titleCell = worksheet.getCell("A1");
    titleCell.value = "Inventory In Report";
    ...
    const headerRow = worksheet.addRow(["No", "Inventory In", "Date", "Vendor", "Total"]);
  ...
  const handlePreview = async (fromDate, toDate, vendorId) => {
    ...
    navigate("/preview-inventory-in", { state: { reportData: data, fromDate, toDate } });
  };
              ...
              <Grid item xs={4}>
                <MDBox mb={2}>
                  <MDInput
                    ...
                    label="Date From"
                    fullWidth
                    value={inventoryIn.dateFrom}
                    onChange={(e) => setInventoryIn({ ...inventoryIn, dateFrom: e.target.value })}
                    InputLabelProps={{
              ...
              <Grid item xs={4}>
                <MDBox mb={2}>
                  <MDInput
                    ...
                    label="Date To"
                    fullWidth
                    value={inventoryIn.dateTo}
                    onChange={(e) => setInventoryIn({ ...inventoryIn, dateTo: e.target.value })}
                    InputLabelProps={{
                      shrink: true, // Ensures label stays on top even when the input is empty
                    }}
                  />
